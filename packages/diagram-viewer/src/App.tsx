...
export function DiagramViewer({ manifest: manifestProp }: { manifest?: DiagramManifest } = {}) {
  const [manifest, setManifest] = useState<DiagramManifest | null>(manifestProp ?? null);

  // Adopt a new manifest passed from the host app (file-drop still replaces it).
  useEffect(() => {
    if (manifestProp) setManifest(manifestProp);
  }, [manifestProp]);
  ...
  // Highlight the node + its directly-connected neighbours.
  const focus = locked;
  ...
  return (
    ...
    >
      <div className="toolbar">
        {!__STANDALONE__ && <span className="title">다이어그램 뷰어</span>}
        {manifest && (
          <span className="meta">
            {manifest.title} · {manifest.kind} · 노드 {manifest.nodes.length}개
          </span>
        )}
        ...
        <div className="dropdown" ref={downloadRef}>
          ...
          {downloadOpen && (
            <div className="dropdown-menu">
              ...
              {/* TODO: "HTML (viewer)" export removed — it depended on a prebuilt
                  standalone viewer.html that isn't shipped in the embedded build. */}
              <button
                onClick={() => {
                  if (!manifest) return;
      ...
      <div className="canvas">
        ...
        {!manifest ? (
          <div className="empty">
            <div>매니페스트 파일을 여세요 (드래그 앤 드롭도 됩니다).</div>
            <div style={{ fontSize: 12 }}>스킬이 만든 DiagramManifest JSON을 렌더링합니다.</div>
          </div>
        ) : (
          <ReactFlow
            nodes={displayNodes}
